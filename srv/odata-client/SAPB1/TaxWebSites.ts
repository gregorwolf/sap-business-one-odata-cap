...
import type { TaxWebSitesApi } from './TaxWebSitesApi';
import {
  PurchaseQuotations,
 ...
 */
export class TaxWebSites<T extends DeSerializers = DefaultDeSerializers>
  extends Entity
  implements TaxWebSitesType<T>
{
  /**
   ...
   * @nullable
   */
  webSiteName?: DeserializedType<T, 'Edm.String'> | null;
  /**
   * Web Site Url.
   * @nullable
   */
  ...
  drafts!: Drafts<T>[];
  /**
   * One-to-many navigation property to the {@link TaxInvoiceReport} entity.
   */
  taxInvoiceReport!: TaxInvoiceReport<T>[];
  /**
  ...
  constructor(readonly _entityApi: TaxWebSitesApi<T>) {
    super(_entityApi);
  }
}
...
> {
  ...
  drafts: DraftsType<T>[];
  taxInvoiceReport: TaxInvoiceReportType<T>[];
