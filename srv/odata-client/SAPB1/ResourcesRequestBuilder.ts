...
> extends RequestBuilder<Resources<T>, T> {
   ...
   * @param code Key property. See {@link Resources.code}.
   ...
   */
  delete(code: string): DeleteRequestBuilder<Resources<T>, T>;
  /**
   * Returns a request builder for deleting an entity of type `Resources`.
  ...
  delete(codeOrEntity: any): DeleteRequestBuilder<Resources<T>, T> {
    return new DeleteRequestBuilder<Resources<T>, T>(
      this.entityApi,
      codeOrEntity instanceof Resources ? codeOrEntity : { Code: codeOrEntity! }
    );
  }
}
